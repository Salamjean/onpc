    ...
    <div class="px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-20 -mb-px">
            ...
            <div class="flex items-center">
                <!-- Hamburger button -->
                <button class="lg:hidden text-gray-400 hover:text-onpc-orange focus:outline-none focus:ring-2 focus:ring-onpc-orange/50 rounded-lg p-2 transition-colors bg-gray-50/50" @click.stop="sidebarOpen = !sidebarOpen" aria-controls="sidebar" :aria-expanded="sidebarOpen">
            ...
            <div class="flex items-center space-x-4">
                ...
                <div class="relative inline-flex" x-data="{ open: false }">
                    <button class="inline-flex justify-center items-center group focus:outline-none" aria-haspopup="true" @click.prevent="open = !open" :aria-expanded="open">
                        <div class="w-10 h-10 rounded-full bg-onpc-blue text-white flex items-center justify-center font-bold border-2 border-gray-100 group-hover:border-onpc-orange transition-colors shadow-sm">
                            {{ substr(Auth::guard('structure')->user()->nom, 0, 1) }}
                        </div>
                        <div class="flex items-center truncate ml-3 hidden md:flex">
                            <span class="truncate text-sm font-semibold text-gray-700 group-hover:text-onpc-orange transition-colors">{{ Auth::guard('structure')->user()->nom }}</span>
                    ...
                    <div class="origin-top-right z-10 absolute top-full right-0 min-w-[200px] bg-white border border-gray-100 py-2 rounded-2xl shadow-xl overflow-hidden mt-2"
                        ...
                        <div class="px-4 py-3 border-b border-gray-50 bg-gray-50/50">
                            <p class="text-sm font-medium text-gray-900 truncate">{{ Auth::guard('structure')->user()->nom }}</p>
                            <p class="text-xs text-gray-500 truncate mt-0.5">{{ Auth::guard('structure')->user()->email }}</p>
                        </div>
                        <ul class="py-2">
                            <li>
                        ...
                        <div class="border-t border-gray-100 pt-1">
                            <form method="POST" action="{{ route('structure.auth.logout') }}" x-data>
                                @csrf
