...
@section('content')
    <div class="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
            <h1 class="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-onpc-blue to-blue-600">
                Sinistres non terminés</h1>
            <p class="text-gray-500 mt-2 font-medium">Suivez les incidents en attente ou en cours d'intervention.</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-8">
        <form action="{{ route('admin.sinistre.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 w-full">
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="status"
                        ...
                        <option value="en_cours" {{ request('status') == 'en_cours' ? 'selected' : '' }}>En cours
            ...
            <div class="flex gap-2">
                ...
                <button type="submit"
                    class="bg-onpc-blue text-white px-8 py-3.5 rounded-2xl font-bold shadow-lg shadow-onpc-blue/20 hover:bg-blue-700 transition-all flex items-center gap-2">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <tbody class="divide-y divide-gray-50">
                    @forelse($sinistres as $sinistre)
                        <tr class="hover:bg-blue-50/30 transition-all duration-200 group">
                            ...
                            <td class="py-5 px-6">
                                ...
                                @else
                                    <div class="flex flex-col items-center gap-1">
                                        ...
                                        @if($sinistre->casernes->count() > 0)
                                            <span class="text-[8px] text-gray-300 font-bold uppercase">{{ $sinistre->casernes->count() }} alertées</span>
                                        @endif
                                    </div>
                                @endif
                            </td>
                            ...
                            @else bg-gray-100 text-gray-600 @endif">
                                    @if ($sinistre->status == 'en_attente' || is_null($sinistre->status))
                                        Alerte reçue
                        ...
                        <tr>
                            <td colspan="6" class="py-20 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 font-bold uppercase tracking-widest text-sm">Aucun sinistre ne
                                        correspond à vos critères.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
